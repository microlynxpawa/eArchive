...
function togglePinnedName() {
  if (!pinTitle) return; // nothing to do if the UI element is not present
...
pinIcon.forEach((item, index) => {
  try {
    const span = item.parentNode && item.parentNode.querySelector ? item.parentNode.querySelector("span") : null;
    var linkName = span ? span.innerHTML : `pin-${index}`;
    var InitialLocalStorage = JSON.parse(localStorage.getItem("pins") || false);

    if (InitialLocalStorage && InitialLocalStorage.includes(linkName) && item.parentNode) {
      item.parentNode.classList.add("pined");
    }
    item.addEventListener("click", (event) => {
      var localStoragePins = JSON.parse(localStorage.getItem("pins") || false);
      if (item.parentNode) item.parentNode.classList.toggle("pined");

      if (localStoragePins?.length) {
        if (item.parentNode && item.parentNode.classList.contains("pined")) {
      ...
      togglePinnedName();
      setTimeout(() => {
        const wrapper = document.getElementsByClassName("simplebar-content-wrapper")[0];
        if (!wrapper) return;
        if (item.parentElement && item.parentElement.parentElement && item.parentElement.parentElement.classList.contains("pined")) {
          scrollTo(wrapper, topPos - 30, 400);
        } else {
          scrollTo(wrapper, elem.parentNode ? elem.parentNode.offsetTop - 30 : topPos - 30, 400);
        }
      }, 200);
    });
  } catch (e) {
    // Defensive: ignore individual pin errors so the rest of the sidebar works
    console.warn('pin icon init failed', e);
  }
